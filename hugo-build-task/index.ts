...
async function run() {
    try {
        ...
        // download version
        const hugoVersion: string | undefined = tl.getInput("hugoVersion", false);
        const extendedVersion: boolean = tl.getBoolInput('extendedVersion', false);

        await getHugo(hugoVersion, extendedVersion);

        const source: string | undefined = tl.getPathInput('source', true, false);
        const destination: string | undefined = tl.getPathInput('destination', true, false);
        const baseURL: string | undefined = tl.getInput('baseURL', false);
        ...
        const minify: boolean = tl.getBoolInput('minify', false);
        const additionalArgs: string | undefined = tl.getInput('additionalArgs', false);
...
async function getHugo(version: string | undefined, extendedVersion: boolean): Promise<void> {
    const latest = 'latest';
    if (!version) {
        version = latest;
    }
